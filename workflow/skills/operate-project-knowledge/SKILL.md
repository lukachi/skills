...
| Settle the decisions behind a direction before anything is written | Invoke `grill-project-decisions`, paired with `model-project-domain`. It maps the open decisions as a tree and asks the whole frontier in numbered rounds with a recommendation on each, until the maintainer confirms a shared understanding. Nothing enters a contract before they do. |
| Make a design question concrete enough to react to | Invoke `prototype-project-decision`. Throwaway code that answers one question, kept as a primary source on its own branch and linked from the record that asked. |
| Explain a flow, a layout, a sequence, or a change to one | Invoke `show-project-work` and draw it. A call tree, a file tree, a component tree, a diff of any of those, a diagram, or one focused HTML artifact — whichever is smallest for the point. Prose is the wrong medium for a structure, and a shortened description of one loses the substance rather than the length. |
