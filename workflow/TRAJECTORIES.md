...
So the packet is generated by `wfctl knowledge trajectory ask`, which cannot print
an address because it never reads one, renders each cause as what it means rather
than as its schema token, and states a limit shared by every finding once instead
of thirteen times.

Generating covers the part that can be generated. The proposal and the cost are
composed, as is every other maintainer-facing message this workflow produces, and
those are governed by the reader test rather than by a check:

> Would the maintainer have to look something up to understand this?

A first attempt guarded the prose fields with patterns instead. It was wrong twice
over. It false-flagged ordinary English — *effaced* is seven hexadecimal
characters, *COVID-19* is an uppercase word with a number — while a record written
in Russian or Japanese naming the same files would pass untouched, because the
patterns test the shape of a string and the rule is about meaning. And it guarded
a layer that was already clean: the measurement that started this found zero
addresses in the records. The failure was in the message, and a message is
composed freely, so the space of ways to get it wrong has no boundary to enumerate.

What replaces it is a stated line, the same one everywhere, named at each point
where a skill produces maintainer-facing text — and the audit that found only one
of nineteen skills naming it at all.

Addresses are not the enemy and must not be removed from the corpus — `evidence`,
`resource` and `edges` are where traceability lives. Nor is the answer to strip
substance along with the address: a sentence with nothing to look up and nothing
to decide on is the over-correction, and it fails the same reader.
