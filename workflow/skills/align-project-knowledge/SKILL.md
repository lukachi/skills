...
populated `knowledge/`, and a populated corpus can still hold nothing about this
Area. A curated page can also record `realization.alignment: drifted` — an intent
the project accepted and its implementation does not deliver. Neither is an error
and neither is reported as a clean result:
[an empty corpus, and a corpus that records a gap](references/gaps-and-drift.md).
